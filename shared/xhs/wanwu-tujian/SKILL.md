...
description: >
  ...
  （干净暖色宣纸底、工笔博物画风、毛笔书法标题、红色方印角标+散布印章），内置 2 种风格变体 ×
  4 种构图模式 × 3 种版面。当用户提到"万物图鉴"、"复古图鉴"、"图鉴风格"、"百科风插画"、
...
## 核心设计：Style × Composition × Layout（3轴系统 v3.1）

三个可组合维度，根据内容自动推荐最优组合：

| 维度 | 控制内容 | 选项 |
|------|----------|------|
| **Style** | 插画风格+色彩能量+写实度 | 2 种风格变体 |
...
- **角标印章**：红色方块印章（左上+右上），内含2-4字中文
- **散布印章**：额外 2-3 个红色印章分散于画面，朱砂红贯穿全图
- **署名**：作者：@知渡
- **背景**：干净暖色宣纸底(#F5EED8)，微妙纸纹，**无重度做旧**
- **封面**：dense-cluster 密集簇拥布局，铺满画布
...
| Composition | 中文名 | 适用场景 | 核心视觉 | 封面子模式 |
|-------------|--------|----------|----------|------------|
| `group-portrait` | 群像集合式 | 品种/分类/大全/文化合集 | 中央大主体+周围6-12小主体 | dense-cluster |
| `center-radial` | 中心辐射式 | 规则/方法/哲理（最高互动） | 中央核心概念+放射子元素 | dense-cluster |
| `infographic` | 信息图表式 | 教程/步骤/指南 | 步骤流程+左右分栏 | dense-cluster |
| `anatomy-atlas` | 博物解剖式 | 单品深度解析/结构分析 | 中央主体+标注线+剖面图 | dense-cluster |

> **v3.0 合并**：`scattered-icons` → center-radial 子模式 B；`grid-collage` → group-portrait 子模式 C

详细定义：`references/compositions/<composition>.md`

---

## Layout Gallery（v3.0 精简）

**竖版 Portrait (1792×2400 / 3:4)**：
- `dense-cluster`（封面专用，密集铺满）
...
## Auto Selection（3轴自动推荐 — v3.1 含写实度）

| 内容信号 | Style | 写实度 | Composition | Cover Layout |
|----------|-------|--------|-------------|--------------|
| 星座/MBTI/性格/人格类型 | traditional-encyclopedia | 75-80% | center-radial | dense-cluster |
| 福报/功德/因果/修行/天规/佛道 | traditional-encyclopedia | 70-75% | center-radial | dense-cluster |
...
| 规则/行为/方法/秘诀/X种/X个 | traditional-encyclopedia | 70-75% | center-radial | dense-cluster |
| 步骤/教程/怎么做/流程/指南 | traditional-encyclopedia | 70-75% | infographic | dense-cluster |
| 人生/哲理/道理/准则/十则 | traditional-encyclopedia | 70-75% | center-radial | dense-cluster |
| 合集/国粹/民俗/习俗/文化 | traditional-encyclopedia | 75-80% | group-portrait | dense-cluster |
...
确定：内容类型 / Hook / 受众 / 风格变体 / **写实度** / 构图 / 翻页流。

### Step 2: Confirmation 1 ⚠️

AskUserQuestion: 风格变体 / 构图 / 方向 / 规模。不可跳过。
...
色彩能量: [NATURAL/WARM]
构图: [composition name]
版面: 封面 dense-cluster / 内容页 organic-poster
路径: [directory path]
图片: N 张
```
...
## Composition × Layout Compatibility（v3.0 精简）

|  | dense-cluster (封面) | organic-poster (内容) | annotated (内容) | side-by-side | panoramic |
|--|:---:|:---:|:---:|:---:|:---:|
| group-portrait | ✓✓ | ✓✓ | ✓ | ✓ | ✓✓ |
| center-radial | ✓✓ | ✓✓ | ✓✓ | ✓ | — |
| infographic | ✓✓ | ✓✓ | ✓ | — | — |
...
## Series Design Principles（v3.1 更新）
...
**Elements** (视觉元素):
- `elements/illustration-styles.md` — 2种风格变体定义 + realism_level 参数（v3.1）
- `elements/canvas.md` — 画布规格、安全区、按风格信息密度
- `elements/color-palettes.md` — 色彩体系、红色分布系统（v3.1）
- `elements/typography.md` — 7级文字层级、毛笔书法标题（v3.1）
- `elements/decorations.md` — 知识图解叠层、扩展印章系统、金属闪光、散落元素（v3.1）

**Compositions** (构图模式 — 4种):
- `compositions/group-portrait.md` — 群像集合式（含 dense-cluster + grid-panel 子模式）
- `compositions/center-radial.md` — 中心辐射式（含 scattered-concept 子模式）
- `compositions/infographic.md` — 信息图表式
- `compositions/anatomy-atlas.md` — 博物解剖式

**Layouts** (版面):
- `layouts/portrait-layouts.md` — organic-poster + dense-cluster + annotated
- `layouts/landscape-layouts.md` — side-by-side + panoramic

**Workflows** (工作流):
- `workflows/analysis-framework.md` — 内容分析框架（含写实度推荐）
- `workflows/outline-template.md` — 大纲模板（含封面 dense-cluster 规划）
- `workflows/prompt-assembly.md` — 提示词组装（9模块，含 v3.1 扩展）
...
- **封面必须使用 dense-cluster 布局**（v3.0 规定）
