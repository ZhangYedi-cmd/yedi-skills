...
> 视觉设计理据见 `elements/` 目录，构图布局见 `compositions/` 目录。
...
Module 2: 背景层
Module 3: 双边框系统
Module 4: 构图主体（人物+物品+动态）    ← 从 compositions/{chosen}.md 加载
Module 5: 文字系统                      ← 规范见 elements/typography.md
Module 6: 印章系统                      ← 规范见 elements/stamps.md
Module 7: 装饰层                        ← 规范见 elements/decorations.md
Module 8: 微观细节                      ← 规范见 elements/micro-details.md
...
**⚠️ 关键**：必须包含 "polished digital illustration"、"smooth"、"silky-smooth"。
...
从 `compositions/{chosen}.md` 加载对应构图 Prompt 片段，替换所有 `{变量}`。
...
| `{style_block}` | `styles/{chosen}.md` | 加载对应风格片段 |

---

## Module 5: 文字系统

> 详细规范见 `elements/typography.md`。
...
> 详细规范见 `elements/micro-details.md`。
...
- Flat ink-line art with watercolor wash (too sketchy)
