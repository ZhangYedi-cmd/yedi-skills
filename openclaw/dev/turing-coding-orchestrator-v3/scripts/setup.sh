...
# =============================================================================
# Usage: setup.sh <task_id> <branch> <worktree_dir> [task_desc]
#
...
# Arguments
# ---------------------------------------------------------------------------
TASK_ID="${1:?Usage: setup.sh <task_id> <branch> <worktree_dir> [task_desc]}"
BRANCH="${2:?Usage: setup.sh <task_id> <branch> <worktree_dir> [task_desc]}"
WORKTREE_DIR="${3:?Usage: setup.sh <task_id> <branch> <worktree_dir> [task_desc]}"
...
if command -v jq &>/dev/null; then
    ...
    python3 -c "
import json, os
f = os.environ['ACTIVE_TASKS_FILE']
with open(f) as fh:
    data = json.load(fh)
tid = os.environ['TASK_ID']
if not any(t['task_id'] == tid for t in data['tasks']):
    data['tasks'].append({
        'task_id': tid,
        'session': tid,
        'branch': os.environ['BRANCH'],
        'worktree': os.environ['WORKTREE_DIR'],
        'started_at': os.environ['TIMESTAMP'],
        'status': 'pending',
        'backend': 'acpx',
