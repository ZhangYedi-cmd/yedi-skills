#!/usr/bin/env python3
"""
watchdog.py — Turing Coding Orchestrator v2
Zero-token background monitor for ACPX/tmux coding agent sessions.
...
Environment variables (all optional with fallbacks):
    ...
    OPENCLAW_GATEWAY_TOKEN   — gateway auth token for notifications
...
ACTIVE_STATES = {TaskState.RUNNING, TaskState.REPAIRING}
...
class Config:
    ...
    def load(cls) -> "Config":
        ...
        # Gateway token: env > config file
        gateway_token = os.environ.get("OPENCLAW_GATEWAY_TOKEN", "")
        if not gateway_token:
            cfg_path = os.environ.get(
                "OPENCLAW_CONFIG_PATH",
                str(Path.home() / ".clawdbot" / "openclaw.json")
            )
            gateway_token = cls._read_gateway_token(cfg_path)
        ...
        return cls(
            ...
            callback_retry_limit=int(os.environ.get("CALLBACK_RETRY_LIMIT", "2")),
    ...
    @staticmethod
    def _read_gateway_token(cfg_path: str) -> str:
        try:
    ...
    def validate(self) -> List[str]:
        ...
        if not self.gateway_token:
            warnings.append("OPENCLAW_GATEWAY_TOKEN not set — notifications will fail")
...
class AcpxClient:
    ...
    def _run(self, args: list[str], timeout: int = 15) -> Optional[str]:
        """Run acpx command in the task's worktree. Returns stdout or None on failure."""
        cmd = ["acpx", self._agent] + args
        cwd = str(self.task.worktree_path) if self.task.worktree_path.exists() else None
        try:
            result = subprocess.run(
                cmd,
                capture_output=True,
    ...
    def read_history(self, tail: int = 50) -> Optional[str]:
        return self._run(["sessions", "read", "--tail", str(tail), self.task.session], timeout=15)

    def get_status(self) -> Optional[str]:
        """Returns agent status string: running|idle|dead or None."""
        out = self._run(["-s", self.task.session, "status"], timeout=10)
        if out is None:
            return None
    ...
    def send_prompt(self, message: str) -> bool:
        """Send a follow-up prompt to the session. Returns True on success."""
        result = subprocess.run(
            ["acpx", "--approve-all", self._agent, "prompt",
...
class Notifier:
    ...
    def _notify_broadcast(self, text: str):
        logger.info(f"notify: broadcast via system event (token={'set' if self.config.gateway_token else 'NOT SET'})")
        env = os.environ.copy()
...
class Watchdog:
    ...
    def run(self):
        ...
        logger.info(f"  TOKEN:       {'set' if self.config.gateway_token else 'NOT SET'}")
        ...
        for task in tasks:
            if task.backend == "acpx":
                ...
                if client.session_alive():
                    logger.info(f"Recovery: {task.task_id} (acpx) — session alive.")
                else:
    ...
    def _monitor_acpx(self, task: Task) -> bool:
        ...
        history = client.read_history(tail=100)
        if history:
            ...
            if cb:
                ...
                if fp in seen:
                    ...
                    return (fresh.state in TERMINAL_STATES) if fresh else True
                seen.add(fp)
    ...
    def _handle_callback(self, task: Task, client: AcpxClient, cb: dict) -> bool:
        ...
        if status == "completed" and failed == 0:
            ...
            self._transition(task, TaskState.COMPLETED, f"Completed: {summary}")
            self.notifier.notify(f"Task {task.task_id} completed: {summary}")
            return True
    ...
    # ------------------------------------------------------------------
    # tmux monitoring (basic)
    # ------------------------------------------------------------------

    @staticmethod
    def _tmux_prompt_client(session: str) -> "TmuxPromptClient":
        return TmuxPromptClient(session)

    def _monitor_tmux(self, task: Task) -> bool:
        if not self._tmux_alive(task.session):
            logger.warning(f"{task.task_id}: tmux session not found → crashed.")
            self._mark_crashed(task)
            return True
        ...
        if output and "callback-json" in output:
            ...
            if cb:
                ...
                seen.add(fp)
                self.memory.write_milestone(task.task_id, "Agent completed (callback detected)")
                client = TmuxPromptClient(task.session)
                return self._handle_callback(task, client, cb)
...
# ---------------------------------------------------------------------------
# TmuxPromptClient — send follow-up prompts via tmux send-keys
# ---------------------------------------------------------------------------
class TmuxPromptClient:
    """Minimal prompt client for tmux-backed sessions."""
    TMUX_SOCKET = "/tmp/openclaw-tmux/openclaw.sock"

    def __init__(self, session: str):
        self.session = session

    def send_prompt(self, msg: str) -> bool:
        try:
            result = subprocess.run(
                ["tmux", "-S", self.TMUX_SOCKET, "send-keys", "-t", self.session, msg, "Enter"],
                capture_output=True,
            )
            return result.returncode == 0
        except Exception as e:
            logger.warning(f"TmuxPromptClient.send_prompt failed: {e}")
            return False
